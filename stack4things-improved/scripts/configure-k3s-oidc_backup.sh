...
CA_SOURCE_PATH="<absolute_path>/s4t_crossplane_deployment/stack4things-improved/keycloak-kestone-integration/certificate/ca.crt"
CA_DEST_PATH="/etc/rancher/k3s/keycloak-ca.crt"
...
if [ -f "$CA_SOURCE_PATH" ]; then
    cp "$CA_SOURCE_PATH" "$CA_DEST_PATH"
    echo -e "${GREEN}✔ Certificato CA copiato${NC}"
else
    echo -e "${RED}❌ ERROR: Certificato non trovato in $CA_SOURCE_PATH${NC}"
    exit 1
fi
